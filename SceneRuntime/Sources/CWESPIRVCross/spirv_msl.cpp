	...
	for (const auto &spv_func : spv_function_implementations)
	{
		...
		case SPVFuncImplArrayCopyMultidim:
		{
			...
			for (uint32_t variant = 0; variant < 12; variant++)
			{
				...
				// Uniform arrays are emitted as spvUnsafeArray<> members of a
				// constant buffer. Array-copy lowering still classifies the source
				// as POD because its backing variable is block-like, so the call
				// reaches this helper without selecting `.elements`. Accept the
				// actual constant-buffer ABI instead of producing invalid MSL.
				if (!is_multidim && variant == 0)
				{
					statement("template<typename T, size_t N>");
					statement("inline void spvArrayCopyFromConstantToStack(",
					          "thread T (&dst)[N], constant spvUnsafeArray<T, N>& src)");
					begin_scope();
					statement("for (uint i = 0; i < N; i++)");
					begin_scope();
					statement("dst[i] = src[i];");
...
string CompilerMSL::argument_decl(const SPIRFunction::Parameter &arg)
{
	...
	if (arg.alias_global_variable && var.basevariable)
	{
		// Helper parameters synthesized from globals must preserve the global
		// resource's address space. In particular, legacy GLSL plain uniforms
		// arrive at the Metal entry point in constant memory and cannot bind to
		// the thread references inferred for the synthesized leaf parameter.
		auto &global_var = get<SPIRVariable>(var.basevariable);
		auto &global_type = get<SPIRType>(global_var.basetype);
		address_space = get_type_address_space(global_type, global_var.self);
	}
